  ...
  const hardcoverPros = [
    {
      ...
      description: (
        <p className='text-[#666]'>
          Some readers are willing to pay for the durability and prestige associated with a hardcover book. Since they’re more expensive to produce, the list price of a hardcover book is typically higher than a paperback. IngramPublisher authors can calculate the compensation of a paperback vs. hardcover book with IngramPublisher’s
    ...
    <div className='max-w-[78.125rem] flex flex-col items-start md:gap-4 xl:gap-7 mx-auto my-12 overflow-x-hidden'>
      <div className="max-w-[765px] w-full md:max-w-[48.75rem] md:w-[clamp(20rem,50vw,48.75rem)] py-6 px-4 sm:px-6 md:px-10 box-border">
        ...
        {SectionOne.map((section, index) => (
          <div key={index} className="space-y-6 mb-15">
            ...
            {section.highlightBox && (
              <div className="border border-gray-200 rounded-lg p-6 bg-[#f6f8fa] flex flex-col xl:flex-row gap-6 items-center">
                {/* IMAGE PLACEHOLDER */}
                <div className="w-[200px] xl:w-[210px] h-[150px] shrink-0 relative">
        ...
        <div className="space-y-6 mb-15">
          ...
          {/* description */}
          <div className='flex flex-col-reverse items-center gap-y-4 xl:items-start xl:grid grid-cols-[1fr_auto]'>
            <ul className='text-[#666]'>
              {SectionTwo.paragraphs}
            </ul>
            <div className='relative w-50 md:w-[clamp(200px,15vw,280px)] h-50 md:g-68'>
              <Image src="/hardcover-paperback-image.webp" alt='image' fill className='object-contain' />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          {hardcoverPros.map((item, idx) => (
            <div key={idx} className="flex gap-1 sm:gap-4 items-start">
              <div className="shrink-0 w-6 h-6 flex items-center justify-center bg-[#629cb1] text-sm rounded-full text-white font-semibold">
                <p>{item.number}</p>
              </div>
              <div>
          ...
          <p className='text-[#666]'>IngramPublisher author, Savannah J. Goins, shared her experience{" "}
            <Link href={"/"} className='text-[#3a7c92] underline hover:text-[#4e9eb8] font-bold'>publishing a hardcover</Link>
            book with IngramPublisher’s Jacketed Case Laminate.
          </p>
          {/* video */}
